...
- **Prettier** (`prettier`, `prettier-plugin-tailwindcss`, `eslint-plugin-prettier`) removed from devDependencies
- **ESLint TypeScript plugins** (`@eslint/js`, `typescript-eslint`, `@typescript-eslint/eslint-plugin`, `@typescript-eslint/parser`, `globals`) removed — coverage moved to Biome
- **`eslint-plugin-anti-trojan-source`** removed — pre-commit standalone CLI hook provides identical coverage
- **`.prettierrc`** config file removed
- **`.prettierignore`** file removed
- **VSCode extensions.json** — `esbenp.prettier-vscode` recommendation removed

### Added

- **6 new and extended unit test files** (37 total test files, ~365 tests):
...
- **Trivy** removed entirely from the project (scanner, config files, CI jobs, npm scripts)
  - Deleted `trivy.yaml`, `trivy-secret.yaml`, `.trivyignore`
  - Deleted `.github/workflows/trivy-scan.yml`
  - Removed Trivy job from `security-full.yml` and `security-scan.yml`
  - Removed `security:trivy` npm script from `package.json`
  - Updated `labeler.yml`, `copilot-instructions.md`, docs to remove Trivy references
  - Helm inline `# trivy:ignore:*` comments retained (harmless); an alternative scanner (Grype/Snyk) will be configured separately
...
- Gitleaks and OpenSSF Scorecard steps in `security-full.yml` now use `continue-on-error: true` to relax non-blocking CI gates (informational alerts no longer fail the pipeline hard)
...
- nginx `nginx.conf` comment updated to clarify WebSocket origin production guidance
...
- **New overrides**: `cross-spawn>=7.0.6` (ReDoS), `protobufjs>=7.5.5` (prototype pollution), `undici>=7.0.0` (SSRF/request-smuggling)
- JWT entropy validation warns operators about weak secrets before they cause incidents
- Dedicated brute-force protection on auth endpoints (10 req/min per IP)
...
[Unreleased]: https://github.com/qnbs/Nexus-HEMS-Dash/compare/v4.5.0...HEAD
[4.5.0]: https://github.com/qnbs/Nexus-HEMS-Dash/compare/v4.4.0...v4.5.0
[4.4.0]: https://github.com/qnbs/Nexus-HEMS-Dash/compare/v1.0.0...v4.4.0
